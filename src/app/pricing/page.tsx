  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      <div className="text-center mb-14">
        ...
        <p className="text-eq-muted text-lg max-w-xl mx-auto">
          ...
          return (
            <div
              ...
              className={`relative rounded-2xl p-7 flex flex-col ${
                popular
                  ? 'border-gradient bg-eq-card shadow-2xl shadow-eq-violet/15 scale-105'
                  : 'border border-eq-border bg-eq-card'
              }`}
            >
              {popular && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 bg-eq-violet rounded-full text-xs font-bold text-white whitespace-nowrap">
                  ⭐ Le plus populaire
                </div>
              )}
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                  popular ? 'bg-eq-violet text-white' : 'bg-eq-surface border border-eq-border text-eq-muted'
                }`}>
                  <Icon className="w-5 h-5" />
                </div>
                <div>
                  <div className="font-bold text-eq-text">{name}</div>
                  <div className="text-xs text-eq-muted">{description}</div>
                </div>
              </div>

              <div className="mb-6">
                {price === 0 ? (
                  <div className="text-4xl font-black text-eq-text">Gratuit</div>
                ) : (
                  <div className="flex items-end gap-1">
                    <span className="text-4xl font-black text-eq-text">{price}€</span>
                    <span className="text-eq-muted mb-1.5 text-sm">/mois</span>
                  </div>
                )}
              </div>
              ...
              <ul className="space-y-3 mb-8 flex-1">
                {features.map((f, i) => (
                  <li key={i} className="flex items-start gap-2.5 text-sm">
                    <Check className="w-4 h-4 text-eq-green shrink-0 mt-0.5" />
                    <span className="text-eq-muted-bright">{f}</span>
                  </li>
                ))}
              </ul>
              ...
              <Link
                ...
                className={`block text-center font-bold py-3 rounded-xl text-sm transition-all ${
                  popular
                    ...
                    : 'bg-eq-surface border border-eq-border text-eq-text hover:border-eq-border-bright hover:bg-eq-card-hover'
                }`}
              >
                {cta}
              </Link>
      ...
      <div className="bg-eq-card border border-eq-border rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-eq-border">
          <h2 className="font-bold text-eq-text">Comparaison détaillée</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-eq-border">
                <th className="text-left px-6 py-4 text-eq-muted font-medium w-1/2">Fonctionnalité</th>
                {PLANS.map(({ name, popular }) => (
                  <th key={name} className={`text-center px-4 py-4 font-bold ${popular ? 'text-eq-violet-light' : 'text-eq-text'}`}>
                    {name}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {FEATURE_ROWS.map(({ label, key, format }, i) => (
                <tr key={key} className={`border-b border-eq-border/50 ${i % 2 === 0 ? '' : 'bg-eq-surface/30'}`}>
                  <td className="px-6 py-3.5 text-eq-muted-bright">{label}</td>
                  {PLANS.map(({ id }) => {
                    ...
                    return (
                      <td key={id} className="px-4 py-3.5 text-center">
                        {isCheck ? <Check className="w-4 h-4 text-eq-green mx-auto" /> :
                         isCross ? <X className="w-4 h-4 text-eq-muted mx-auto" /> :
                         <span className={`font-semibold ${id === 'free' ? 'text-eq-muted' : 'text-eq-text'}`}>{formatted}</span>}
                      </td>
                    )
                  })}
                </tr>
